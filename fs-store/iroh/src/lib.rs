...
impl Client {
    ...
    async fn list_stream_in_model(&self, model_id: &StreamId) -> anyhow::Result<Vec<FileSlot>> {
...
mod tests {
    ...
    async fn operate_stream() -> anyhow::Result<()> {
        ...
        let mut stream =
            FileSlot::new(&dapp_id, genesis.r#type, &commit, state.must_model().ok()).unwrap();
