...
use dataverse_file_types::core::{dapp_store, stream::FileSlotStore};
...
pub async fn migration(cfg: &Config, operator: Arc<dyn StreamOperator>) -> anyhow::Result<()> {
    if let Ok(migration) = std::env::var("MIGRATION") {
        ...
        if split.iter().any(|&s| s == "stream_store") {
            ...
            migration_file_store(operator, iroh_store, pgsql_store).await?;
        }
    };
    Ok(())
}

async fn migration_file_store(
    operator: Arc<dyn StreamOperator>,
    from: Arc<dyn FileSlotStore>,
    to: Arc<dyn FileSlotStore>,
) -> anyhow::Result<()> {
